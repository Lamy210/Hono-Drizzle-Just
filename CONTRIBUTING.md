...
- Mutable tenant-owned sample resources must preserve optimistic-concurrency semantics: emit a strong ETag for an individual representation and require `If-Match` for PATCH and DELETE. Keep PATCH version matching plus version increment in the same database UPDATE, and DELETE version matching in the same database DELETE. Do not replace either with an application read-then-write check or an unscoped existence probe.
