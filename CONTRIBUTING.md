...
- Idempotency handling must remain tenant-scoped and transactional. Hash raw keys before persistence, never add raw keys/hashes/fingerprints to logs or telemetry, and replay users only through tenant-scoped repository methods. The current user-create ledger is pointer-only: same-fingerprint replay resolves the current tenant-local representation for the original user ID. Physical expiry cleanup must remain bounded and index-backed: preserve the shared process cleanup gate, finite batch size, `FOR UPDATE SKIP LOCKED`, and `(expires_at, tenant_id, key_hash)` index rather than adding an unbounded hot-path delete. Do not add response snapshots or mutable user PII to the ledger without an explicit retention/privacy/versioning design.
