...
- Mutable tenant-owned sample resources must preserve optimistic-concurrency semantics: emit a strong ETag for an individual representation, require `If-Match` for PATCH, and keep the version predicate plus version increment in the same database UPDATE. Do not replace this with an application read-then-write check or an unscoped existence probe.
