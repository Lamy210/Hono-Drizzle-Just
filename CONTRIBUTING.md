...
- Cursor pagination remains tenant-scoped and keyset-based. Keep opaque cursor encoding in the presentation layer, keep tenant IDs out of cursor payloads, preserve the `created_at DESC, id DESC` tie-break, and do not replace the repository predicate with deep OFFSET pagination.
- Mutable tenant-owned sample resources must preserve ETag semantics: emit a strong ETag for an individual representation, support `If-None-Match` weak-comparison revalidation on GET after the normal authenticated tenant-scoped lookup, and require `If-Match` for PATCH and DELETE. Keep PATCH/DELETE precondition evaluation, tenant-local existence classification, and the mutation in one database statement snapshot. Do not reintroduce a zero-row write followed by a separate existence SELECT, an application read-then-write check, or an unscoped existence probe.
- Idempotency handling must remain tenant-scoped and transactional. Hash raw keys before persistence, never add raw keys/hashes/fingerprints to logs or telemetry, and replay users only through tenant-scoped repository methods. The current user-create ledger is pointer-only: same-fingerprint replay resolves the current tenant-local representation for the original user ID. Physical expiry cleanup must remain bounded, index-backed, and outside the idempotency business transaction: preserve the shared process cleanup gate, root-database maintenance adapter, finite batch size, `FOR UPDATE SKIP LOCKED`, and `(expires_at, tenant_id, key_hash)` index rather than adding an unbounded hot-path delete or moving maintenance back into a claim/complete transaction. Do not add response snapshots or mutable user PII to the ledger without an explicit retention/privacy/versioning design.
...
- Telemetry attributes must remain low-cardinality and must not contain SQL bind values, credentials, email addresses, UUIDs, or other sensitive/request-specific values. Idempotency cleanup metrics are intentionally bounded to backend/operation/result; never add tenant IDs, keys, hashes, fingerprints, user IDs, or error text.
