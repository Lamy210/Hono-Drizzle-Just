...
User deletion uses a separate `UserDeleteRepository` and the same version precondition model as update. `DELETE /users/{id}` requires `If-Match`; the adapter combines tenant ID, user ID, and accepted version in one `DELETE ... RETURNING id`, so a stale representation cannot delete a newer user state. A zero-row conditional delete performs only a tenant-scoped existence check to distinguish stale 412 from missing/cross-tenant 404. `If-Match: *` keeps the standard existence-only semantics and intentionally does not protect against an intervening version change. Completed `user_creation_idempotency` rows reference `users.id` with `ON DELETE CASCADE`; a successful conditional delete removes its replay pointer atomically in PostgreSQL, so a previously completed idempotency key can be claimed again afterward.

User updates use a separate application-owned `UserUpdateRepository`. `UpdateUserService` derives tenant ownership from the principal, canonicalizes the UUID, normalizes changed fields, and requires an optimistic-concurrency precondition. Individual create/get/update responses expose a strong ETag such as `"v1"`; `PATCH /users/{id}` requires `If-Match`, returns 428 when it is omitted, and returns 412 when no supplied strong validator matches the current user version. The Drizzle implementation performs the version predicate and `version = version + 1` in the same tenant-scoped `UPDATE ... RETURNING`, preventing lost updates without a read-before-write race. A zero-row update performs a tenant-scoped existence check only to distinguish cross-tenant/missing 404 from stale 412. Weak ETags do not satisfy `If-Match`; lists and `*` follow RFC conditional-request semantics. PostgreSQL remains authoritative for tenant-local email uniqueness, and a matching-version email collision maps to the same sanitized 409 conflict as create.
