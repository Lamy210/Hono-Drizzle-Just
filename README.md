...
The PostgreSQL `user_creation_idempotency` ledger and user insert are owned by the same transaction. A successful claim has a 24-hour replay window measured with database time. Expired entries are still reclaimed immediately when that tenant/key is reused. In production composition, a shared in-process cleanup gate additionally allows at most one bounded cleanup attempt per process every 60 seconds; the query deletes at most 1,000 expired rows using `FOR UPDATE SKIP LOCKED` and the `(expires_at, tenant_id, key_hash)` cleanup index. This prevents one-time keys from accumulating forever without requiring Redis or a separate background worker. Multiple replicas can clean different expired batches without concentrating on the same locked rows. Cleanup emits bounded custom metrics: `idempotency.cleanup.runs` carries only backend, operation, and `success`/`error`; `idempotency.cleanup.rows` accumulates successfully deleted rows. Tenant IDs, keys, hashes, fingerprints, user IDs, and error text are never metric attributes. Expiry only permits the key to be treated as new again—it does not bypass ordinary tenant-local email uniqueness, so retrying an already-created email after expiry may return `409`.
