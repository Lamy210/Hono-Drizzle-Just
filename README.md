...
- `GET /users/cursor` - protected tenant-scoped keyset listing; requires `users:read`; supports opaque `cursor` plus `limit` (1-100, default 20); returns `nextCursor` or `null` and uses `Cache-Control: private, no-store`; when another page exists, the response also carries an RFC 8288 `Link: <...>; rel="next"` header
...
Successful user mutations also use `Cache-Control: private, no-store`: `POST /users`, `PATCH /users/{id}`, and `DELETE /users/{id}` are tenant-scoped write results and are not retained for later reuse. `POST`/`PATCH` still return their ETag/Location metadata where applicable; clients that want a cacheable representation re-fetch the canonical `GET /users/{id}` resource, which uses the separate `private, no-cache` revalidation model.

Tenant-scoped user lists deliberately use `Cache-Control: private, no-store`. Unlike individual users, the sample list endpoint does not expose a collection ETag, so retaining the response provides no validator-based revalidation benefit while increasing the risk of stale tenant data remaining in browser/intermediary caches. Hono's automatic `HEAD /users` handling inherits the same non-storage policy.

Individual user reads support HTTP cache revalidation without weakening tenant isolation. `GET /users/{id}` emits the same strong ETag used by conditional writes and accepts `If-None-Match`. After the normal authenticated tenant-scoped lookup succeeds, the route applies weak entity-tag comparison: both `"vN"` and `W/"vN"` match the current version, while `*` matches any current authorized representation. A match returns 304 with the current ETag and no body; a stale or unrelated tag returns the normal 200 representation. Missing and cross-tenant users remain 404 before validator evaluation.

User deletion uses a separate `UserDeleteRepository` and the same version precondition model as update. `DELETE /users/{id}` requires `If-Match`; the adapter evaluates tenant ownership, resource existence, version matching, and deletion inside one PostgreSQL data-modifying CTE statement. The statement returns `deleted`, `precondition_failed`, or `not_found` from one MVCC snapshot, so stale 412 versus missing/cross-tenant 404 classification has no second-query race window. `If-Match: *` keeps the standard existence-only semantics and intentionally does not protect against an intervening version change. Completed `user_creation_idempotency` rows reference `users.id` with `ON DELETE CASCADE`; a successful conditional delete removes its replay pointer atomically in PostgreSQL, so a previously completed idempotency key can be claimed again afterward.

User updates use a separate application-owned `UserUpdateRepository`. `UpdateUserService` derives tenant ownership from the principal, canonicalizes the UUID, normalizes changed fields, and requires an optimistic-concurrency precondition. Individual create/get/update responses expose a strong ETag such as `"v1"`; `PATCH /users/{id}` requires `If-Match`, returns 428 when it is omitted, and returns 412 when no supplied strong validator matches the current user version. The Drizzle implementation evaluates tenant ownership, current existence, the version predicate, field changes, and `version = version + 1` inside one PostgreSQL data-modifying CTE statement. It returns `updated`, `precondition_failed`, or `not_found` from one MVCC snapshot, eliminating the earlier zero-row UPDATE followed by an existence SELECT race. Weak ETags do not satisfy `If-Match`; lists and `*` follow RFC conditional-request semantics. PostgreSQL remains authoritative for tenant-local email uniqueness, and a matching-version email collision maps to the same sanitized 409 conflict as create.

The sample list API uses bounded offset pagination because the repository already ships a reusable page/per-page contract. Results are ordered by `created_at DESC, id DESC` for a deterministic tie-break, and the response shape is `{ data, meta: { page, perPage, total, totalPages } }`. The tenant total and requested page are read by one PostgreSQL statement, so `data` and `meta.total` come from the same MVCC snapshot instead of two independently committed read statements.

For larger tenant collections, the additive `GET /users/cursor` endpoint demonstrates keyset pagination without changing the existing page/per-page contract. The opaque cursor contains only a versioned `createdAt + id` position, never tenant data. Repository lookup uses the same tenant predicate and descending composite index with `(created_at, id) < (cursor.createdAt, cursor.id)`, requests `limit + 1` rows to derive `nextCursor`, and therefore avoids deep OFFSET scans. The `users_tenant_created_id_idx` B-tree index matches the query as `(tenant_id, created_at DESC NULLS FIRST, id DESC NULLS FIRST)`, so the tenant predicate is the leading equality key and PostgreSQL can preserve the list ordering from the index. The explicit `NULLS FIRST` is intentional: the current Drizzle index builder otherwise defaults descending index columns to `NULLS LAST`, while `desc(column)` queries use PostgreSQL's bare-DESC `NULLS FIRST` semantics. Page numbers are capped at 10,000 and page size at 100 to prevent unbounded client-selected offsets/page sizes. Offset pagination can still shift while concurrent writes occur and becomes less efficient for very deep datasets; generated services with large or high-write collections should replace this sample query with cursor/keyset pagination rather than simply raising the cap.
...
`POST /users` accepts an optional `Idempotency-Key` header. When omitted, user creation keeps the ordinary tenant-local behavior. When present, the key is scoped to the authorized tenant: the same tenant/key with the same normalized `email` and `name` resolves the same user ID and returns HTTP `201`, while reusing an active tenant/key for a different normalized payload returns HTTP `422` with `IDEMPOTENCY_KEY_REUSED`. The ledger stores a resource pointer rather than a historical response snapshot, so if that user is later changed through `PATCH /users/{id}`, a subsequent matching replay returns the **current tenant-scoped user representation and current ETag** for that same ID. Fresh creates and successful replays both return `Location: /users/{id}`, so clients can resolve the primary user resource without reconstructing its URI from response-body conventions. The same raw key may be used independently by another tenant.
...
The PostgreSQL `user_creation_idempotency` ledger and user insert are owned by the same transaction. A successful claim has a 24-hour replay window measured with database time. Expired entries are still reclaimed immediately when that tenant/key is reused. Physical cleanup is a separate best-effort maintenance operation on the root database connection, outside the idempotent-create transaction. In production composition, a shared in-process cleanup gate allows at most one bounded cleanup attempt per process every 60 seconds; the query deletes at most 1,000 expired rows using `FOR UPDATE SKIP LOCKED` and the `(expires_at, tenant_id, key_hash)` cleanup index. This prevents one-time keys from accumulating forever without requiring Redis or a separate background worker. Multiple replicas can clean different expired batches without concentrating on the same locked rows. Cleanup emits bounded custom metrics: `idempotency.cleanup.runs` carries only backend, operation, and `success`/`error`; `idempotency.cleanup.rows` accumulates successfully deleted rows. Tenant IDs, keys, hashes, fingerprints, user IDs, and error text are never metric attributes. Cleanup failure is reported through those metrics and the database DELETE span but is not allowed to abort the following idempotency transaction; claim/complete correctness remains authoritative inside its own transaction. Expiry only permits the key to be treated as new again—it does not bypass ordinary tenant-local email uniqueness, so retrying an already-created email after expiry may return `409`.
...
The port may optionally return quota metadata. The PostgreSQL fixed-window adapter does so, allowing the HTTP boundary to publish `RateLimit-Policy` and `RateLimit` response fields on completed allowed requests and on direct 429 responses. Reset/retry delays are calculated from PostgreSQL timestamps only: the upsert returns both bucket expiry and `clock_timestamp()`, so application-host clock skew cannot inflate or truncate `RateLimit`/`Retry-After` timing. The emitted syntax follows `draft-ietf-httpapi-ratelimit-headers-11` (May 2026), for example `RateLimit-Policy: "http.users.write";q=30;w=60` and `RateLimit: "http.users.write";r=29;t=60`. That document is still an Internet-Draft rather than an RFC, so these fields are explicitly provisional; `Retry-After` remains the authoritative wait instruction on 429. Adapters that omit quota metadata continue to work and emit none of the provisional fields.
...
When `HTTP_RATE_LIMIT_ENABLED=true`, production composition installs the shared PostgreSQL fixed-window adapter using the global default policy plus the configured users read/write overrides. The adapter stores one row per `(scope, identity_hash)`, atomically increments or resets that row with PostgreSQL upsert semantics, and therefore shares counters across replicas that use the same database. The raw client address is never persisted: the stored identity is SHA-256 of the scope plus the normalized identity. Expired identities are cleaned periodically on the request path in batches of at most 1,000 rows. Cleanup uses `FOR UPDATE SKIP LOCKED` plus an `(expires_at, scope, identity_hash)` index so concurrent replicas avoid concentrating on the same expired rows and one request never performs an unbounded delete. Cleanup is retention maintenance rather than quota correctness: a cleanup DELETE failure is observed and suppressed, after which the authoritative quota mutation still runs. An expired row for an active identity is reused rather than creating one row per window. A scope without an explicit override falls back to the global policy.

The PostgreSQL adapter remains fail-closed for authoritative quota reads/writes: database/adapter failures during the consume decision follow the common internal-error path instead of silently disabling abuse protection. The only fail-open exception is expired-row cleanup maintenance, because failure to vacuum stale buckets does not make an otherwise valid quota decision less authoritative. Deployments that require a different latency or availability trade-off can replace the `RateLimiter` port with Redis or an edge-backed implementation without changing the HTTP contract. A process-local Map is intentionally not the production default because counters would split across replicas. The OpenAPI user routes advertise 429 because enabling the port changes their observable response surface.

When telemetry is enabled, the production adapter emits `rate_limit.decisions` and `rate_limit.decision.duration`. Cleanup additionally emits `rate_limit.cleanup.runs` with bounded `success`/`error` and `rate_limit.cleanup.rows` for successfully reclaimed rows. Attributes are deliberately bounded to backend (`postgresql`), algorithm (`fixed_window` or `gcra`), and the relevant finite result dimension. Client addresses, identity hashes, scopes, tenant IDs, retry delays, and exception text are not metric labels.


All structured application error responses use `Cache-Control: no-store`. This includes validation/authentication/authorization failures, tenant-local 404s, optimistic-concurrency errors, rate-limit 429s, internal errors, and normalized routing 404/405 responses. Error payloads are point-in-time and may depend on authenticated tenant/session state, so the template does not allow browsers or intermediary caches to retain and replay them.
...
Successful sample-user mutations emit application-owned business events after persistence succeeds: `user.created`, `user.updated`, and `user.deleted`. These events carry only `userId`, `requestId`, and `traceId`; email, name, tenant ID, request bodies, idempotency keys, hashes, and fingerprints are not copied into the event context. Failed authorization, validation, not-found, and persistence operations do not emit a successful mutation event. Idempotent create replay also remains silent so one committed creation produces one `user.created` event.
...
PostgreSQL pool state is exported only when telemetry collection asks for it, using the application-owned `ObservableMeter` extension rather than sampling pool state on request paths. Production composition registers the default pool as `db.client.connection.pool.name=primary` and reports `db.client.connection.count` with `idle` / `used` state attributes, `db.client.connection.max`, and `db.client.connection.pending_requests`. The adapter uses observable up/down counters to match the current OpenTelemetry database pool conventions; connection strings, host names, database names, and request-derived values are not metric attributes. With `OTEL_ENABLED=false`, `NoopMeter` registers no callback and does not evaluate pool state. The same pool boundary also records `db.client.connection.wait_time` in seconds for successful acquisitions and increments `db.client.connection.timeouts` only when obtaining a connection fails because of a timeout. Because node-postgres `Pool.query()` acquires through `Pool.connect()`, these measurements cover Drizzle queries, transactions, direct pool queries, and readiness checks without repository-specific instrumentation. New-connection acquisitions additionally populate `db.client.connection.create_time`, while each borrowed connection records `db.client.connection.use_time` exactly once when its wrapped `release()` returns it to the pool. This separates slow connection establishment from long-held checked-out connections without adding query text or request data to metric attributes.
