...
- `GET /users/cursor` - protected tenant-scoped keyset listing; requires `users:read`; supports opaque `cursor` plus `limit` (1-100, default 20); returns `nextCursor` or `null` and uses `Cache-Control: private, no-store`
