...
Deletion uses a separate `UserDeleteRepository` application port and a single tenant-scoped `DELETE ... RETURNING id` statement. The application service translates a false delete result to the same tenant-local 404 used by reads/updates. No pre-delete existence probe is performed. The completed idempotency ledger foreign key uses `ON DELETE CASCADE`, so deleting a user and removing its replay pointer remain one PostgreSQL referential action; a later request may claim the old key again rather than replaying a deleted resource.
