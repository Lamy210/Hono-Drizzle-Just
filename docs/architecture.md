...
Rate limiting is fail-closed in the selected PostgreSQL adapter for authoritative quota state: INSERT/UPDATE/SELECT failures used to decide allow/deny propagate through the common HTTP 500 path instead of treating the request as allowed. Expired-bucket cleanup is a separate best-effort maintenance exception. Both adapters schedule a bounded cleanup before quota mutation, but a cleanup DELETE failure is recorded and suppressed so stale-row retention cannot deny an otherwise valid request. This preserves the security boundary while isolating non-authoritative maintenance from request availability. Applications that need fail-open behavior, lower-latency Redis counters, token buckets, sliding windows, or provider-edge enforcement can still replace the application-owned `RateLimiter` port without changing request identity resolution or the HTTP 429 contract.
...
Rate-limit observability is a separate `RateLimitObserver` rather than a concern embedded in the HTTP middleware or database observer. It emits one decision counter and one decision-duration histogram per consume attempt, including failures. The same observer owns `rate_limit.cleanup.runs` and `rate_limit.cleanup.rows`; cleanup run labels add only a bounded `success`/`error` result. The only attributes are the finite adapter backend, algorithm, and result dimensions. Scope, client address, persisted identity hash, tenant data, retry delay, deleted bucket keys, and exception details are intentionally excluded to prevent PII leakage and unbounded metric cardinality.
...
Successful mutation responses are also intentionally non-storable. `POST /users`, `PATCH /users/{id}`, and `DELETE /users/{id}` set `Cache-Control: private, no-store` because they are tenant-scoped write results rather than reusable cache representations. Create/update may still expose `Location`/ETag metadata; the canonical read surface remains `GET /users/{id}`, whose `private, no-cache` policy permits storage only with mandatory revalidation.
