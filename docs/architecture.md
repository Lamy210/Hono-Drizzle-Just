...
Database execution safety is enforced at the connection configuration boundary rather than in repository code. The production pool passes a server-side PostgreSQL `statement_timeout` (default 15 seconds) and `idle_in_transaction_session_timeout` (default 30 seconds) to every application session. `statement_timeout` cancels only the overlong statement and leaves a healthy connection reusable; the idle-in-transaction timeout terminates the session so locks and transaction snapshots are not held indefinitely. Both settings accept `0` to disable them explicitly. They are separate from `connectionTimeoutMillis`, which bounds obtaining or establishing a pool client before any SQL can run.
